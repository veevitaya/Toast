...
- **Admin Color Scheme:** OKLCH-based palette via CSS custom properties in `index.css` `:root`. Primary purple `--admin-blue` (oklch 0.56 0.27 295 ≈ #A05AFF), coral `--admin-pink` (oklch 0.72 0.14 18 ≈ #FE9496), cyan `--admin-cyan` (oklch 0.78 0.12 215 ≈ #4BCBEB), teal `--admin-teal` (oklch 0.76 0.14 175 ≈ #1BCFB4), deep purple `--admin-deep-purple` (oklch 0.54 0.28 295 ≈ #9E58FF). Plus light/opacity variants. Yellow `#FFCC02` for brand elements (nav, sidebar, Butters). Owner portal accent: green `#00B14F`.

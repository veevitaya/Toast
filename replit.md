...
- **Admin Color Scheme:** CSS custom properties in `index.css` `:root`. Indigo `--admin-deep-purple` (#6366F1), rich blue `--admin-blue` (#3B7BF5), coral `--admin-pink` (#FF6347), cyan `--admin-cyan` (#4DD9D0), soft violet `--admin-teal` (#A78BFA). Charts use indigo-to-blue gradient (`linear-gradient(90deg, var(--admin-deep-purple), var(--admin-blue))`). Yellow `#FFCC02` for sidebar/nav/Butters ONLY — removed from all graphs. Owner portal accent: green `#00B14F`.

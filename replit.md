...
- **Admin Color Scheme:** OKLCH-based palette via CSS custom properties (`--admin-blue`, `--admin-pink`, `--admin-cyan`, `--admin-blue-light`, `--admin-pink-light`, plus opacity variants `--admin-blue-10`, `--admin-blue-90`, `--admin-pink-10`, `--admin-cyan-20`, `--admin-cyan-30`). Defined in `index.css` `:root`. Yellow `#FFCC02` for brand elements (nav, sidebar, Butters). Owner portal accent: green `#00B14F`.

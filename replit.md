...
- **Contact & Partnerships System:** Public `/contact` page with mascot-driven landing (toast/waffle/popcorn/ticket characters), 4 routing cards (User Feedback, Restaurant/Food Partner, Events/Activities, Other Partnerships), 5-step user feedback survey with progress dots and localStorage draft, and 3 concise partner forms with optional expandable details. Mascots are scoped to `/contact` only — never in the main app. Server enforces honeypot spam-blocking, rate limit (5/min/IP), 5MB-per-file limit (max 5 files as base64 data URLs), and auto-priority rules (low satisfaction/ease scores, mall/franchise/multi-branch businesses, and investor/sponsorship/data partnerships are auto-tagged "high"). Admin dashboard at `/admin/contact-submissions` (superadmin-only) provides tabbed table (All/per-type/Archived), filters (status/priority/lead quality/has-files/search/date), CSV export, side-drawer detail with Contact Info / Submission Info / per-type lead summary / Internal Controls (status/priority/lead quality/notes/tags) / Activity Log, and archive/unarchive. Tables: `contact_submissions` (with `metadata` jsonb for category-specific fields), `contact_submission_files` (base64 data URLs), `contact_submission_activity` (audit log).

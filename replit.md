...
- **Admin Color Scheme:** CSS custom properties in `index.css` `:root`. Purple `--admin-deep-purple` (#8B5CF6), blue `--admin-blue` (#4A7BF7), hot pink `--admin-pink` (#F24B8A), cyan `--admin-cyan` (#4DD9D0), soft pink `--admin-teal` (#F0B0D0). Charts use purple-to-blue gradient (`linear-gradient(90deg, var(--admin-deep-purple), var(--admin-blue))`). Yellow `#FFCC02` for sidebar/nav/Butters ONLY — removed from all graphs. Owner portal accent: green `#00B14F`.

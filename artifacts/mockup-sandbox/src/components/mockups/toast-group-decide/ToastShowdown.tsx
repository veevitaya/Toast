...
import { ArrowLeft, Hand, Scissors, Grab, Check, Crown, Lock, RotateCcw, Soup, Pizza, Flame, Utensils } from "lucide-react";
...
type Phase = "intro" | "choose" | "countdown" | "round" | "decide" | "result";
...
export default function ToastShowdown() {
  ...
  const [chosenDish, setChosenDish] = useState<Dish | null>(null);
  ...
  // Mint auto-picks when Mint won the showdown
  useEffect(() => {
    if (phase !== "decide" || matchWinner !== "mint") return;
    const t = setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      {/* Scoreboard — visible once the match is on, hidden during the countdown so the reveal isn't spoiled */}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex-1 flex flex-col">
            <div className="text-center mb-7">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mb-4" style={{ backgroundColor: "rgba(26,26,26,0.04)" }}>
                ...
                <span className="text-[12px] font-bold uppercase tracking-wider">Winner chooses</span>
              </div>
              <h1 className="font-['Plus_Jakarta_Sans'] text-[28px] font-bold tracking-tight leading-tight">
                Settle it with a showdown
              </h1>
              <p className="text-[15px] mt-2 leading-relaxed px-2" style={{ color: "rgba(26,26,26,0.6)" }}>
                You and Mint both liked 4 spots. Rock, paper, scissors — best of 3. The winner picks where you eat.
              </p>
            </div>
            ...
            <div className="bg-white rounded-[20px] border border-black/[0.04] shadow-[0_8px_20px_-14px_rgba(0,0,0,0.12)] p-4">
              <span className="text-[11px] font-bold tracking-[0.14em] uppercase" style={{ color: MUTE }}>On the line</span>
            ...
            {matchWinner === "you" ? (
              <>
                <div className="text-center mb-6 mt-2">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: "rgba(255,204,2,0.2)", color: GOLD }}>
                  ...
                  <p className="text-[14px] mt-2" style={{ color: MUTE }}>
                    Your call — pick where you two are eating.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {DISHES.map((d) => (
                    <button
                      ...
                      onClick={() => { setChosenDish(d); setPhase("result"); }}
                      className="bg-white rounded-[20px] overflow-hidden shadow-[0_10px_24px_-16px_rgba(0,0,0,0.2)] border border-black/[0.04] text-left active:scale-[0.97] transition-transform hover:shadow-[0_14px_28px_-14px_rgba(255,204,2,0.45)]"
                    >
                      <div className="h-[96px] flex items-center justify-center" style={{ backgroundColor: d.tint }}>
              ...
              <div className="flex-1 flex flex-col items-center justify-center pb-16">
                <div className="relative mb-8">
                  ...
                  <div className="w-20 h-20 rounded-full flex items-center justify-center relative z-10" style={{ backgroundColor: "#F3F1EC", color: INK, boxShadow: "0 12px 30px -12px rgba(0,0,0,0.2)" }}>
                    <span className="font-['Plus_Jakarta_Sans'] text-[24px] font-bold">M</span>
                  </div>
                </div>
                <h2 className="font-['Plus_Jakarta_Sans'] text-[22px] font-bold text-center">Mint won the showdown</h2>
                <p className="text-[14px] mt-2" style={{ color: MUTE }}>Mint is choosing for you both...</p>
              </div>
            )}
            <div className="mt-auto" />
          </div>
          ...
          <div className="animate-in zoom-in-95 fade-in duration-600 flex-1 flex flex-col pt-2">
            <div className="flex items-center gap-2 justify-center mb-5">
              ...
              <span className="text-[13px] font-bold tracking-[0.15em] uppercase">
                {matchWinner === "you" ? "Your pick" : "Mint's pick"}
              </span>
            </div>

            <div className="bg-white rounded-[28px] overflow-hidden shadow-[0_18px_40px_-18px_rgba(0,0,0,0.16)] border border-black/[0.05]">
              <div className="relative h-[200px] flex items-center justify-center" style={{ backgroundColor: chosenDish.tint }}>
                <chosenDish.Icon className="w-24 h-24" strokeWidth={1.25} style={{ color: INK, opacity: 0.55 }} />
              ...
              <div className="p-6">
                <h2 className="font-['Plus_Jakarta_Sans'] text-[26px] font-bold tracking-tight leading-tight">{chosenDish.name}</h2>
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <p className="text-[14px] font-semibold" style={{ color: INK }}>
                    {matchWinner === "you" ? "You" : "Mint"} won {myScore}–{oppScore} and chose
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 flex flex-col gap-2 items-center">
              <span className="text-[12px] font-medium" style={{ color: MUTE }}>From your 4 mutual likes</span>
              <div className="flex gap-1.5">
                {DISHES.filter((d) => d.id !== chosenDish.id).map((d) => (
      ...
      {/* ACTION AREA */}
      <div
        className={`fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto p-6 pb-10 transition-all duration-500 ${phase === "countdown" || (phase === "decide" && matchWinner === "mint") ? "translate-y-full opacity-0 pointer-events-none" : "translate-y-0 opacity-100"}`}
      ...
      >
        {phase === "intro" && (
          ...
          >
            <Scissors className="w-5 h-5" /> Start the showdown
          </button>
        )}
        ...
        {phase === "round" && (
          <button
            ...
            onClick={() => {
              ...
              else if (matchWinner) setPhase("decide");
              else nextThrow();
            }}
            className="w-full h-14 rounded-full font-bold text-[16px] flex items-center justify-center gap-2 active:scale-[0.98] transition-transform shadow-[0_8px_20px_-8px_rgba(255,204,2,0.55)]"
            style={{ backgroundColor: GOLD, color: INK }}
          >
            {roundWinner === "tie" ? "Throw again" : matchWinner ? "See who chooses" : "Next round"}
          </button>
        )}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5">
        <span className="w-6 h-6 rounded-full flex items-center justify-center font-['Plus_Jakarta_Sans'] text-[10px] font-bold" style={{ backgroundColor: "#F3F1EC", color: INK }}>{badge}</span>
        <span className="text-[12px] font-bold" style={{ color: MUTE }}>{initial}</span>
      </div>
      <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ backgroundColor: win ? "rgba(255,204,2,0.18)" : "#F7F4EE" }}>
        <T.Icon className="w-8 h-8" strokeWidth={1.6} style={{ color: INK }} />
      </div>

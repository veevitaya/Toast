...
import { Database, AlertTriangle, CheckCircle, Search, RefreshCw, Trash2, Image, Tag, DollarSign, Link2, Clock } from "lucide-react";
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
          {(["issues", "sync", "validation"] as const).map(t => (
            <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-1.5 text-sm rounded-lg font-medium transition-all capitalize ${activeTab === t ? "bg-white text-gray-800 shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6" data-testid="card-quick-actions">
            ...
            <div className="space-y-3">
              {[
                { label: "Auto-assign missing vibe tags", desc: "Uses keyword + price + hours logic", icon: Tag },
                { label: "Validate all delivery URLs", desc: "Check Grab/LINE MAN/Robinhood links", icon: Link2 },
                { label: "Flag stale restaurants", desc: "Mark records not updated in 90+ days", icon: Clock },
                { label: "Merge duplicate entries", desc: "Find & merge by name + location proximity", icon: Trash2 },
                { label: "Re-sync Google Places data", desc: "Pull latest info for all restaurants", icon: RefreshCw },
              ].map(action => (
                <button key={action.label} className="w-full flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors text-left" data-testid={`btn-action-${action.label.toLowerCase().replace(/\s+/g, "-")}`}>

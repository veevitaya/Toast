...
**Why:** these param shapes aren't discoverable from the callback signatures and repeatedly cause failed calls; the shared-`id` pitfall surfaced when two canvas frames pointed at the same app and the consumer frame kept mirroring the other frame's route (e.g. admin login).

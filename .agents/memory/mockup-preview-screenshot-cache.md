...
**Don't over-invest in captures — the blank is genuinely intermittent.** Even with thorough warming (curl 5-6x), captures can come back blank-white for SEVERAL different fresh `?cb=N` in a row while the component is perfectly fine, then suddenly render on the next identical attempt. It is non-deterministic, not a code signal. Once you have ONE good capture of a screen, trust `npx tsc --noEmit` (clean) + HMR-applied-cleanly logs + reasoning for subsequent non-structural edits (CSS class tweaks, `aria-hidden`, etc.) instead of burning many captures chasing a clean re-shot.
